...
To be more thorough, im thinking lets do each point (ie 1-6 above, ill be referring to them as sections 1-6 from now onwards) one at a time instead of dumping the entire solution at once. ill cross question, and try to deep dive on each section until i feel 100% locked in and then we can proceed to next section! 
Further, i feel that i understand better when provided examples, analogies. So try to introduce parallels, examples and analogies whenever/wherever possible (judiciously, dont abuse) to help me locking the concepts.  
Also, We can follow a push (from my end) heavy approach instead of you endlessly pulling (by quizes, tangents) after/before each section (keep this minimal, instead teach me everything inside the section properly/correctly so that we don't need pulls/pushes even (ideally)). Optionally after each section you can suggest some followup questions/deepdive points (related to the section(s) seen already and if i feel necessary, ill ask as a followup, else prompt to move on to next section! )
...
-----------
makes sense!, I think i understand section 5 well now, can you please give me everything you covered till now in this section (ie section 5) as a single click to download .md file(ie for section 5). make in comprehensive, almost a 1:1 copy verbatim to your answer and no trimming truncating or using handwavy/slogan like language(cardinal sin if you do this even after i told not to, since this .md will be the replica in-case the thread goes down!).  
Also, Include the questions and answers to all the optional deep-dive prompts you suggested (short answers, not too detail nor too shallow) at the end of the .md as well please/ 

-----------------------------

lets generate a full markdown cheat sheet spanning all 6 sections to reasonable depth. i want it to be comprehensive, strictly make sure its not slogan like/handwavy, include examples, analogies and core concepts from every section. don't be parsimonious with explaining stuff, this is gods work we have done in this thread and its a sin if you leave it out of the .md and quite dangerous in case the thread gets delete and we lose all these learnings!
Note: give me the .md file as a click to download file please. make it as elaborate, well structured, well articulated, easy to follow and comprehensive as possible, dont disappoint me with a weak, half baked .md which only skims each section in slogan like language.